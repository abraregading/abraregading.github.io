      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <h3 class="card-title">Tabel Dafar Galeri</h3>
          </div>
          <!-- /.card-header -->
          <div class="card-body">
              <a href="/porfolio/create" class="btn btn-sm btn-primary sm-3">Tambah Data</a>
              @if (session('pesan'))
            ...
            <table id="example2" class="table table-bordered table-hover">
              ...
              <tbody>
                  @foreach($porfolio as $data)
              <tr>
                <td width="80">{{ $loop->iteration }}</td>
                <td>{{ $data->kategori->kt }}</td>
                <td>{{ $data->judul }}</td>
                <td width="120">
                    <a href="/porfolio/{{ $data->id }}/edit" class="btn btn-sm btn-info">Edit</a>
...
<div class="modal fade" id="hapus{{ $data->id }}">
    <div class="modal-dialog modal-sm">
        ...
        <div class="modal-footer justify-content-between">
            ...
            <form action="/porfolio/{{ $data->id }}" method="post">
                @method('delete')
                @csrf
